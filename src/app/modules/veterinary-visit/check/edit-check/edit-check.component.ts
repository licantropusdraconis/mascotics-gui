...
export class EditCheckComponent implements OnInit {
  ...
  */
  ...
  ngOnInit(): void {
    ...
    this.SearchMedicalCheck();
  }

  SearchMedicalCheck(){
    this.MedicalCheckService.GetMedicalCheckById(this.id).subscribe((data:MedicalCheckModel)=>{
      //this.fgValidator.controls["id"].setValue(this.id);
      //this.fgValidator.controls["nit"].setValue(data.NIT);
      this.fgValidator.controls["fechaatencion"].setValue(data.FechaAtencion);
      this.fgValidator.controls["temperatura"].setValue(data.Temperatura);
      this.fgValidator.controls["peso"].setValue(data.Peso);
      this.fgValidator.controls["frecuenciacardiaca"].setValue(data.FrecuenciaCardiaca);
      this.fgValidator.controls["frecuenciarespiratoria"].setValue(data.FrecuenciaRespiratoria);
      this.fgValidator.controls["estadoanimo"].setValue(data.EstadoAnimo);
      this.fgValidator.controls["recomendaciones"].setValue(data.Recomendaciones);
      this.fgValidator.controls["medicamentos"].setValue(data.Medicamentos);
      this.fgValidator.controls["solicitudvisitadomiciliariaid"].setValue(data.solicitudVisitaDomiciliariaId);
    });
  }
  ...
  EditMedicalCheck(){
    ...
    this.MedicalCheckService.UpdateMedicalCheck(vt,this.id).subscribe((data:MedicalCheckModel)=>{
      alert(` Medical Check ${fechaatencion} ${solicitudvisitadomiciliariaid} ${recomendaciones} ${medicamentos} updated!`);
      this.router.navigate(["/veterinary-visit/search-check"])
    }, (_error: any)=>{
      alert("Error updating Medical Check");
    }
    )
  }
}
